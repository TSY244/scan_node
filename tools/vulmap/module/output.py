...
def output(types, item):
    try:
        ...
        loguru.logger.error("[ERROR] " + error.__traceback__.tb_frame.f_globals['__file__'])
        # print(now.timed(de=0) + color.red
        # ("[ERROR] " + error.__traceback__.tb_frame.f_globals['__file__']
...
def output_json(filename, data):
    # ?????txt ???????
    string=create_json(data)+"\n"
    with open(filename, 'a', encoding='utf-8') as f:
        f.write(string)
    loguru.logger.info("write json to file")
    ...
    #         else:
    #             with open("ret/"+filename, 'a', encoding='utf-8') as f2:
...
def create_json(vul_info):
    dir_data={}
    dir_data["prt_name"]=vul_info["prt_name"]
    dir_data["vul_name"]=vul_info["vul_name"]
    dir_data["vul_numb"]=vul_info["vul_numb"]
    dir_data["vul_type"]=vul_info["vul_type"]
    dir_data["vul_urls"]=vul_info["vul_urls"]
    dir_data["vul_payd"]=vul_info["vul_payd"]

    return str(json.dumps(dir_data, ensure_ascii=False))